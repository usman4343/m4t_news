...
function article_base_getlinks($keyword, $camp) {
	...
	if( trim($startPageBloked)  != 'yes' ){
		...
		if($page == 0){
			$curlurl="https://startpage.com/do/asearch?hmb=1&cat=web&cmd=process_search&language=english&engine0=v1all&query=$keywordenc&abp=1&nj=0&pg=0";
		}else{
			...
			//if time below one hour use it
				...
				$curlurl="https://{$qid_server}startpage.com/do/search?cmd=process_search&language=english&qid=$qid&rcount=3&rl=NONE&abp=1&query=$keywordenc&cat=web$startTxt&nj=0";
	...
	if( $startPageBloked == 'yes'){
		...
		if($page == 0){
			...
			curl_setopt($this->ch, CURLOPT_URL, "https://duckduckgo.com/html");
			curl_setopt($this->ch, CURLOPT_POSTFIELDS, "q=$keywordenc&b=&kl=us-en");
			curl_setopt($this->ch, CURLOPT_POST, 1);
			
		
			
		}else{
			//get the s and dc var 
			
			$varS = get_post_meta( $camp->camp_id , 'duckDuckGoIndex',1);
			
			if( trim($varS) !='' && is_numeric($varS)){
				
				$varDc = $varS +1;
				
				curl_setopt($this->ch, CURLOPT_URL, "https://duckduckgo.com/html");
				curl_setopt($this->ch, CURLOPT_POSTFIELDS, "q=$keywordenc&s=$varS&nextParams=&v=l&o=json&dc=$varDc&api=%2Fd.js&kl=us-en");
				curl_setopt($this->ch, CURLOPT_POST, 1);
		...
		// s variables for next page
		$varS = '';
		if(stristr($exec,'result__a') ){
			
			preg_match_all('{name="s" value="(.*?)"}', $exec,$sMatchs);
		 	
			if(isset($sMatchs[1][1])){
				$varS = $sMatchs[1][1];
			}else{
				$varS = $sMatchs[1][0];
			}
			...
			if( trim($varS) != '' && is_numeric($varS)){
				echo '<br>Next index for DuckDuckGo is '.$varS;
				update_post_meta( $camp->camp_id , 'duckDuckGoIndex',$varS);
			}
			
		}
		
		//Extracting ezine urls
		preg_match_all('{"result__url" href="(.*?)"}', $exec,$urlsMatchs);
		 
		$foundUrls = $urlsMatchs[1];
	...
	if($startPageBloked != 'yes'){
		...
		if( trim($varS) != '' && is_numeric($varS)){
			...
			delete_post_meta( $camp->camp_id , 'duckDuckGoIndex' );
			$page = -1;
			$this->deactivate_key($camp->camp_id, $keyword);
